...
class AttributesPanel extends Component {
  ...
  // helper function to generate the domain JSON for the updateDomains() action
  ...
  // dispatch REST actions based on which panel is active. includes updateDomains() action 
  _handleSave() {
  ...
  // set the selected record to an empty {}
  _clearEdits() {
    ...
    this.props.setSaveButton();
    this.props.setEditButton("deactivate");
  ...
  render() {
    ...
    if (this.props.isVisible) {
      ...
      const editMode = this.props.editButton;
      const editTooltip = editMode ? "Start Editing" : "Stop and discard edits"
      return (
        <Container>
                ...
                <Card.Header className="bg-light m-1 p-1" style={{ flex: 4 }}>
                  <Row className=" mx-2 p-1">
                    ...
                    <Col style={{ flex: 3, justifyContent: 'right', textAlign: 'right' }}>
                      {!(this.props.card === "projects_overview") &&
                        <Tooltip title={editTooltip}>
                          <Button color="green" clear={!editMode} className="m-1 mx-3 p-1 px-2"
                            onClick={() => this._toggleEditing()}
                            icon={<PencilIcon size={16} filled={editMode} />}
                            iconPosition="before"
                          > Edit
                        </Button>
                        </Tooltip>}
